...
function extractResolversFromSchema(schema, options) {
    ...
    for (const typeName in typeMap) {
        if (!typeName.startsWith('__')) {
            ...
            if (typeDef instanceof graphql.GraphQLScalarType) {
                resolvers[typeName] = typeDef;
            }
            else if (typeDef instanceof graphql.GraphQLObjectType || typeDef instanceof graphql.GraphQLInterfaceType) {
                resolvers[typeName] = extractFieldResolversFromObjectType(typeDef, {
                    selectedTypeDefs: options && options.selectedTypeDefs,
                });
            }
            else if (typeDef instanceof graphql.GraphQLEnumType) {
            ...
            else if (typeDef instanceof graphql.GraphQLUnionType) {
                resolvers[typeName] = {
...
const IGNORED_SCALARS = [graphql.GraphQLBoolean.name, graphql.GraphQLInt.name, graphql.GraphQLString.name, graphql.GraphQLFloat.name, graphql.GraphQLID.name];
function printSchemaWithDirectives(schema, options) {
    ...
    if (allTypesAst.length === 0 && directivesAst.length === 0 && allTypesExtensionAst.length === 0) {
        return graphql.printSchema(schema, options);
    }
    else {
        const astTypesPrinted = [...allTypesAst, ...directivesAst, ...allTypesExtensionAst].map(ast => graphql.print(ast));
        const nonAstPrinted = noAstTypes.map(p => graphql.printType(p, options));
