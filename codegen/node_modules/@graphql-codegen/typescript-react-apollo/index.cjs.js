...
class ReactApolloVisitor extends visitorPluginCommon.ClientSideBaseVisitor {
    constructor(schema, fragments, rawConfig, documents) {
        super(schema, fragments, rawConfig, {
            componentSuffix: visitorPluginCommon.getConfigValue(rawConfig.componentSuffix, 'Component'),
            withHOC: visitorPluginCommon.getConfigValue(rawConfig.withHOC, true),
            withComponent: visitorPluginCommon.getConfigValue(rawConfig.withComponent, true),
            withHooks: visitorPluginCommon.getConfigValue(rawConfig.withHooks, false),
            withMutationFn: visitorPluginCommon.getConfigValue(rawConfig.withMutationFn, true),
            withRefetchFn: visitorPluginCommon.getConfigValue(rawConfig.withRefetchFn, false),
            apolloReactCommonImportFrom: visitorPluginCommon.getConfigValue(rawConfig.apolloReactCommonImportFrom, rawConfig.reactApolloVersion === 3 ? '@apollo/client' : '@apollo/react-common'),
            apolloReactComponentsImportFrom: visitorPluginCommon.getConfigValue(rawConfig.apolloReactComponentsImportFrom, rawConfig.reactApolloVersion === 3 ? '@apollo/client' : '@apollo/react-components'),
            apolloReactHocImportFrom: visitorPluginCommon.getConfigValue(rawConfig.apolloReactHocImportFrom, rawConfig.reactApolloVersion === 3 ? '@apollo/client' : '@apollo/react-hoc'),
            apolloReactHooksImportFrom: visitorPluginCommon.getConfigValue(rawConfig.apolloReactHooksImportFrom, rawConfig.reactApolloVersion === 3 ? '@apollo/client' : '@apollo/react-hooks'),
            reactApolloVersion: visitorPluginCommon.getConfigValue(rawConfig.reactApolloVersion, 2),
            withResultType: visitorPluginCommon.getConfigValue(rawConfig.withResultType, true),
            withMutationOptionsType: visitorPluginCommon.getConfigValue(rawConfig.withMutationOptionsType, true),
    ...
    _buildHocProps(operationName, operationType) {
        const typeVariableName = this._externalImportPrefix + this.convertName(operationName + pascalCase.pascalCase(operationType) + this._parsedConfig.operationResultSuffix);
        const variablesVarName = this._externalImportPrefix + this.convertName(operationName + pascalCase.pascalCase(operationType) + 'Variables');
  ...
  ${propsTypeName}<TChildProps>>) {
    return ApolloReactHoc.with${pascalCase.pascalCase(node.operation)}<TProps, ${operationResultType}, ${operationVariablesTypes}, ${propsTypeName}<TChildProps>>(${this.getDocumentNodeVariable(node, documentVariableName)}, {
    ...
    _buildComponent(node, documentVariableName, operationType, operationResultType, operationVariablesTypes) {
        ...
        const isVariablesRequired = operationType === 'Query' && node.variableDefinitions.some(variableDef => variableDef.type.kind === graphql.Kind.NON_NULL_TYPE);
 ...
 * const [${camelCase.camelCase(operationName)}, { data, loading, error }] = use${operationName}({
 *   variables: {${variableString}
 *   },
 * });`;
        return `
