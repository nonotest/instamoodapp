import { Kind, parse, isSchema, Source } from 'graphql';
import { debugLog, compareNodes, asArray, resolveBuiltinModule, isDocumentString, parseGraphQLSDL, printSchemaWithDirectives, fixSchemaAst, compareStrings } from '@graphql-toolkit/common';
import isGlob from 'is-glob';
import { uniqBy, keyBy, reverse, includes, flatten, isEqual, groupBy } from 'lodash';
import { printWithComments, resetComments, mergeSchemasAsync } from '@graphql-toolkit/schema-merging';
    ...
    for (const pointer in normalizedPointerOptionsMap) {
        ...
        if (isDocumentString(pointer)) {
            loadPromises$.push(Promise.resolve().then(async () => {
                const result = parseGraphQLSDL(`${stringToHash(pointer)}.graphql`, pointer, Object.assign(Object.assign({}, options), pointerOptions));
                found.push(result);
        ...
        if (partialSource) {
            ...
            if (resultSource.rawSDL) {
                if (isEmptySDL(resultSource.rawSDL)) {
                    ...
                    resultSource.document = parse(new Source(resultSource.rawSDL, resultSource.location), specificOptions);
                }
            }
