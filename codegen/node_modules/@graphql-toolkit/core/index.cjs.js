...
function collectNewTypeDefinitions(allDefinitions, definitionPool, newDefinition, schemaMap) {
    ...
    function collectNode(node) {
        ...
        // collect missing argument input types
        if (!definitionPool.some(d => 'name' in d && d.name.value === nodeTypeName) && !lodash.includes(builtinTypes, nodeTypeName)) {
    ...
    function collectDirective(directive) {
        ...
        if (!definitionPool.some(d => 'name' in d && d.name.value === directiveName) && !lodash.includes(builtinDirectives, directiveName)) {
    ...
    if (fs && path) {
        ...
        if (isGraphQLFile(fullPath) && isGraphQLFile(importFrom)) {
            try {
                ...
                if (e.code === 'ENOENT') {
                    const resolveFrom = await new Promise(function (resolve) { resolve(_interopNamespace(require('resolve-from'))); }).then(m => m.default);
 ...
 */
async function collectDefinitions(imports, { location, document, rawSDL }, options, typeDefinitions, allDefinitions) {
    // Add all definitions to running total
    allDefinitions.push(document.definitions);
    // Filter TypeDefinitionNodes by type and defined imports
    const currentTypeDefinitions = filterImportedDefinitions(imports, document.definitions, allDefinitions, options.sort);
    ...
    await Promise.all(rawModules.map(async (m) => {
        ...
        if (!processedFile || !processedFile.find(rModule => lodash.isEqual(rModule, m))) {
...
async function loadTypedefs(pointerOrPointers, options) {
    ...
    for (const pointer in normalizedPointerOptionsMap) {
        ...
        if (common.isDocumentString(pointer)) {
            loadPromises$.push(Promise.resolve().then(async () => {
                const result = common.parseGraphQLSDL(`${stringToHash(pointer)}.graphql`, pointer, Object.assign(Object.assign({}, options), pointerOptions));
                found.push(result);
            ...
            loadPromises$.push(Promise.resolve().then(async () => {
                ...
                if (graphql.isSchema(customLoaderResult && customLoaderResult)) {
                    found.push({
                        location: pointer,
                        schema: customLoaderResult,
                    });
                }
                else if (customLoaderResult && customLoaderResult.kind && customLoaderResult.kind === graphql.Kind.DOCUMENT) {
                    const result = {
        ...
        loadPromises$.push(Promise.resolve().then(async () => {
            ...
            await Promise.all(paths.map(async (path) => {
                if (globOptions.loader) {
                    ...
                    if (graphql.isSchema(customLoaderResult)) {
                        const result = {
                            schema: customLoaderResult,
                            document: graphql.parse(common.printSchemaWithDirectives(customLoaderResult)),
                            location: path,
                        };
    ...
    await Promise.all(found.map(async (partialSource) => {
        if (partialSource) {
            const specificOptions = Object.assign(Object.assign({}, options), (partialSource.location in normalizedPointerOptionsMap ? globOptions : normalizedPointerOptionsMap[partialSource.location]));
            const resultSource = Object.assign({}, partialSource);
            if (resultSource.schema) {
                resultSource.schema = common.fixSchemaAst(resultSource.schema, specificOptions);
                resultSource.rawSDL = common.printSchemaWithDirectives(resultSource.schema, specificOptions);
            }
            if (resultSource.rawSDL) {
                if (isEmptySDL(resultSource.rawSDL)) {
                    ...
                    resultSource.document = graphql.parse(new graphql.Source(resultSource.rawSDL, resultSource.location), specificOptions);
                }
            }
            if (resultSource.document) {
                ...
                if (!resultSource.rawSDL) {
                    resultSource.rawSDL = schemaMerging.printWithComments(resultSource.document);
    ...
    return options.sort ? foundValid.sort((left, right) => common.compareStrings(left.location, right.location)) : foundValid;
}
