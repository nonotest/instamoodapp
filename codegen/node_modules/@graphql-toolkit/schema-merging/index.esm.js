...
import { GraphQLScalarType, getDescription, visit, print, Source, GraphQLSchema, Kind, isSpecifiedScalarType, isIntrospectionType, printType, parse, buildSchema, buildASTSchema } from 'graphql';
import { isNotEqual, fixSchemaAst, asArray, composeResolvers, extractResolversFromSchema } from '@graphql-toolkit/common';
import { addResolveFunctionsToSchema, addErrorLoggingToSchema, SchemaDirectiveVisitor } from '@ardatan/graphql-tools';
...
function mergeSchemas({ schemas, typeDefs, resolvers, resolversComposition, schemaDirectives, resolverValidationOptions, logger, ...config }) {
    const typeDefsOutput = mergeTypeDefs([...schemas, ...(typeDefs ? asArray(typeDefs) : [])], config);
    const resolversOutput = composeResolvers(mergeResolvers([...schemas.map(schema => extractResolversFromSchema(schema)), ...(resolvers ? asArray(resolvers) : [])], config), resolversComposition || {});
...
async function mergeSchemasAsync({ schemas, typeDefs, resolvers, resolversComposition, schemaDirectives, resolverValidationOptions, logger, ...config }) {
    const [typeDefsOutput, resolversOutput] = await Promise.all([
        mergeTypeDefs([...schemas, ...(typeDefs ? asArray(typeDefs) : [])], config),
        Promise.all(schemas.map(async (schema) => extractResolversFromSchema(schema))).then(extractedResolvers => composeResolvers(mergeResolvers([...extractedResolvers, ...(resolvers ? asArray(resolvers) : [])], config), resolversComposition || {})),
    ]);
    let schema = typeof typeDefsOutput === 'string' ? buildSchema(typeDefsOutput, config) : buildASTSchema(typeDefsOutput, config);
    if (resolversOutput) {
        schema = addResolveFunctionsToSchema({
...
export { collectComment, extractType, isGraphQLDirective, isGraphQLEnum, isGraphQLEnumExtension, isGraphQLInputType, isGraphQLInputTypeExtension, isGraphQLInterface, isGraphQLInterfaceExtension, isGraphQLScalar, isGraphQLScalarExtension, isGraphQLSchema, isGraphQLType, isGraphQLTypeExtension, isGraphQLUnion, isGraphQLUnionExtension, isListTypeNode, isNonNullTypeNode, isSchemaDefinition, isSourceTypes, isStringTypes, isWrappingTypeNode, mergeArguments, mergeDirective, mergeDirectives, mergeEnum, mergeEnumValues, mergeFields, mergeGraphQLNodes, mergeGraphQLSchemas, mergeGraphQLTypes, mergeInputType, mergeInterface, mergeNamedTypeArray, mergeResolvers, mergeSchemas, mergeSchemasAsync, mergeType, mergeTypeDefs, mergeUnion, printComment, printTypeNode, printWithComments, pushComment, resetComments };
