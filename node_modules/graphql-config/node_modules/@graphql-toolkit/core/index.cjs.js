...
async function processImportSyntax(documentSource, options) {
    ...
    const firstTypes = lodash.flatten(options.typeDefinitions);
    const secondFirstTypes = options.typeDefinitions[0];
    const otherFirstTypes = lodash.flatten(options.typeDefinitions.slice(1));
    ...
    document.definitions = completeDefinitionPool(lodash.flatten(options.allDefinitions), firstSet, lodash.flatten(options.typeDefinitions));
}
/**
    ...
    for (const pointer in normalizedPointerOptionsMap) {
        ...
        if (common.isDocumentString(pointer)) {
            loadPromises$.push(Promise.resolve().then(async () => {
                const result = common.parseGraphQLSDL(`${stringToHash(pointer)}.graphql`, pointer, options);
                found.push(result);
            ...
            loadPromises$.push(Promise.resolve().then(async () => {
                ...
                if (customLoaderResult && customLoaderResult instanceof graphql.GraphQLSchema) {
                    found.push({
                        location: pointer,
        ...
        loadPromises$.push(Promise.resolve().then(async () => {
            ...
            await Promise.all(paths.map(async (path) => {
                if (globOptions.loader) {
                    ...
                    if (customLoaderResult instanceof graphql.GraphQLSchema) {
                        const result = {
                            schema: customLoaderResult,
    ...
    await Promise.all(found.map(async (partialSource) => {
        if (partialSource) {
            ...
            if (resultSource.schema) {
                resultSource.schema = common.fixSchemaAst(resultSource.schema, options);
                resultSource.rawSDL = common.printSchemaWithDirectives(resultSource.schema);
            }
            if (resultSource.rawSDL) {
                if (isEmptySDL(resultSource.rawSDL)) {
                    ...
                    resultSource.document = graphql.parse(new graphql.Source(resultSource.rawSDL, resultSource.location), options);
                }
            }
...
async function loadSchema(schemaPointers, options) {
    ...
    if (options.resolvers) {
        if (typeof options.resolvers === 'string') {
            resolvers = await fileLoading.loadFilesAsync(options.resolvers);
        }
        else if (options.resolvers instanceof Array) {
            resolvers = schemaMerging.mergeResolvers(await Promise.all(options.resolvers.map(async (r) => (typeof options.resolvers === 'string' ? fileLoading.loadFilesAsync(r) : r))));
        }
    }
