...
function isRootType(type, schema) {
    return graphql.isEqualType(type, schema.getQueryType()) || graphql.isEqualType(type, schema.getMutationType()) || graphql.isEqualType(type, schema.getSubscriptionType());
}
...
class BaseTypesVisitor extends BaseVisitor {
    ...
    getObjectTypeDeclarationBlock(node, originalNode) {
        ...
        const { type } = this._parsedConfig.declarationKind;
        const allFields = [...(this.config.addTypename ? [indent(`${this.config['immutableTypes'] ? 'readonly' : ''} ${optionalTypename}: '${node.name}'${this.getPunctuation(type)}`)] : []), ...node.fields];
...
class BaseDocumentsVisitor extends BaseVisitor {
    ...
    OperationDefinition(node) {
        ...
        const operationTypeSuffix = this.config.omitOperationSuffix ? '' : this.config.dedupeOperationSuffix && name.toLowerCase().endsWith(node.operation) ? '' : pascalCase.pascalCase(node.operation);
...
class BaseResolversVisitor extends BaseVisitor {
    ...
    createResolversFields(applyWrapper, clearWrapper, getTypeToUse) {
        ...
        return typeNames.reduce((prev, typeName) => {
            ...
            if ((shouldApplyOmit && prev[typeName] !== 'any' && graphql.isObjectType(schemaType)) || (graphql.isInterfaceType(schemaType) && !isMapped)) {
...
class ClientSideBaseVisitor extends BaseVisitor {
    ...
    _generateFragment(fragmentDocument) {
        ...
        const isDocumentNode = this.config.documentMode === exports.DocumentMode.documentNode || this.config.documentMode === exports.DocumentMode.documentNodeImportFragments;
    ...
    getImports() {
        ...
        if (this.config.documentMode === exports.DocumentMode.graphQLTag || this.config.documentMode === exports.DocumentMode.documentNodeImportFragments) {
            (this._fragments || [])
    ...
    OperationDefinition(node) {
        ...
        const operationTypeSuffix = this.config.dedupeOperationSuffix && node.name.value.toLowerCase().endsWith(node.operation) ? '' : this.config.omitOperationSuffix ? '' : operationType;
