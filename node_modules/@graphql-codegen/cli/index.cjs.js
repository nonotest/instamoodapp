...
const logUpdate = _interopDefault(require('log-update'));
...
const loadSchema = async (schemaPointers, config) => {
    try {
        const loaders = [new codeFileLoader.CodeFileLoader(), new gitLoader.GitLoader(), new githubLoader.GithubLoader(), new graphqlFileLoader.GraphQLFileLoader(), new jsonFileLoader.JsonFileLoader(), new urlLoader.UrlLoader(), new apolloEngineLoader.ApolloEngineLoader(), new prismaLoader.PrismaLoader()];
...
const hash = (content) => crypto.createHash('sha1')
    .update(content)
    .digest('base64');
async function generate(input, saveToFile = true) {
    ...
    async function writeOutput(generationResult) {
        ...
        await Promise.all(generationResult.map(async (result) => {
            ...
            mkdirp.sync(basedir);
            const absolutePath = path.isAbsolute(result.filename) ? result.filename : path.join(input.cwd || process.cwd(), result.filename);
