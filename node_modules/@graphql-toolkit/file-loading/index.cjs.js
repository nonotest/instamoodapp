...
function loadFiles(pattern, options = LoadFilesDefaultOptions) {
    ...
    const relevantPaths = scanForFiles(asArray(pattern).map(path => (isDirectory(path) ? buildGlob(unixify(path), execOptions.extensions, execOptions.ignoredExtensions, execOptions.recursive) : unixify(path))), options.globOptions);
    ...
    return Promise.all(relevantPaths
        ...
        if (extension.endsWith('.js') || extension.endsWith('.ts') || execOptions.useRequire) {
            ...
            return new Promise(async (resolve, reject) => {
                const { readFile } = await new Promise(function (resolve) { resolve(_interopNamespace(require('fs'))); });
