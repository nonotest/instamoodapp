...
const graphqlTools = require('@ardatan/graphql-tools');
...
const isMergeableObject = (target) => {
    ...
    if (target instanceof graphql.GraphQLScalarType) {
        return false;
    }
    return true;
};
    ...
    const allNodes = types
        ...
        if (isGraphQLSchema(type)) {
            ...
            if (validAstNodes.length === 0 && Object.keys(typesMap).length > 0) {
                schema = common.fixSchemaAst(schema, config);
            ...
            const allTypesPrinted = Object.keys(typesMap)
                ...
                const isPredefinedScalar = type instanceof graphql.GraphQLScalarType && graphql.isSpecifiedScalarType(type);
                const isIntrospection = graphql.isIntrospectionType(type);
                if (isPredefinedScalar || isIntrospection) {
                    return null;
                }
                if (type.astNode) {
                    return graphql.print(type.extensionASTNodes ? extendDefinition(type) : type.astNode);
                }
                else {
                    // KAMIL: we might want to turn on descriptions in future
                    return graphql.printType(correctType(typeName, typesMap), { commentDescriptions: config.commentDescriptions });
                }
            })
                .filter(e => e);
            const directivesDeclaration = schema
                .getDirectives()
                .map(directive => (directive.astNode ? graphql.print(directive.astNode) : null))
                .filter(e => e);
            const printedSchema = [...directivesDeclaration, ...allTypesPrinted, schemaDefinition].join('\n');
            return graphql.parse(printedSchema);
        }
...
function mergeSchemas({ schemas, typeDefs, resolvers, resolversComposition, schemaDirectives, resolverValidationOptions, logger, ...config }) {
    const typeDefsOutput = mergeTypeDefs([...schemas, ...(typeDefs ? common.asArray(typeDefs) : [])], config);
    const resolversOutput = common.composeResolvers(mergeResolvers([...schemas.map(schema => common.extractResolversFromSchema(schema)), ...(resolvers ? common.asArray(resolvers) : [])], config), resolversComposition || {});
    let schema = typeof typeDefsOutput === 'string' ? graphql.buildSchema(typeDefsOutput, config) : graphql.buildASTSchema(typeDefsOutput, config);
    if (resolversOutput) {
    ...
    if (logger) {
        graphqlTools.addErrorLoggingToSchema(schema, logger);
    }
    if (schemaDirectives) {
        graphqlTools.SchemaDirectiveVisitor.visitSchemaDirectives(schema, schemaDirectives);
    }
    return schema;
}
async function mergeSchemasAsync({ schemas, typeDefs, resolvers, resolversComposition, schemaDirectives, resolverValidationOptions, logger, ...config }) {
    const [typeDefsOutput, resolversOutput] = await Promise.all([
        mergeTypeDefs([...schemas, ...(typeDefs ? common.asArray(typeDefs) : [])], config),
        Promise.all(schemas.map(async (schema) => common.extractResolversFromSchema(schema))).then(extractedResolvers => common.composeResolvers(mergeResolvers([...extractedResolvers, ...(resolvers ? common.asArray(resolvers) : [])], config), resolversComposition || {})),
    ]);
    let schema = typeof typeDefsOutput === 'string' ? graphql.buildSchema(typeDefsOutput, config) : graphql.buildASTSchema(typeDefsOutput, config);
    if (resolversOutput) {
        schema = graphqlTools.addResolveFunctionsToSchema({
